document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/static/js/loading.json', { method: 'HEAD' })
        ...
        .catch(() => { /* Lottie file not found, CSS spinner serves as fallback */ });
    ...
    function sanitize(val) {
        ...
        if (s === '' || s.toLowerCase() === 'nan' || s.toLowerCase() === 'none') return 'Vacío';
        return s;
    }
    ...
    async function fetchData() {
        ...
        try {
            ...
            if (auditorFilter.options.length <= 1) {
                Object.keys(cachedData.auditors).sort().forEach(auditor => {
            ...
            alert('Error al conectar con la API. Verifica que el servidor esté corriendo.');
        } finally {
            hideLoading();
        }
    }
    ...
    async function refreshData() {
        ...
        refreshBtn.innerHTML = '<span class="animate-spin inline-block mr-2">⟳</span> Refrescando...';
        showLoading();
    ...
    function updateUI() {
        ...
        if (filter !== 'all' && data.per_auditor_data && data.per_auditor_data[filter]) {
        ...
        // Update cards
        document.getElementById('total-activities').innerText = summary.total_activities;
        document.getElementById('total-errors').innerText = summary.activities_with_errors;
        document.getElementById('clean-activities').innerText = summary.clean_activities;
        document.getElementById('avg-progress').innerText = summary.avg_progress + '%';

        const progBar = document.getElementById('progress-bar');
        progBar.style.width = Math.min(summary.avg_progress, 100) + '%';

        const rowsErr = summary.rows_with_errors || summary.activities_with_errors;
        const errPct = summary.total_activities > 0
            ? ((rowsErr / summary.total_activities) * 100).toFixed(1)
            : 0;
        document.getElementById('error-percentage').innerText = errPct + '% filas con error';
    ...
    function renderAuditorChart(auditors, filter) {
        ...
        const bgColors = labels.map(l => l === filter ? 'rgb(190, 24, 93)' : 'rgba(190, 24, 93, 0.4)');
    ...
    function renderErrorsByAuditorChart(errorsByAuditor, filter) {
        const ctx = document.getElementById('errorsByAuditorChart').getContext('2d');
        if (errorsByAuditorChart) errorsByAuditorChart.destroy();

        const labels = Object.keys(errorsByAuditor);
        const bgColors = labels.map(l => l === filter ? 'rgb(239, 68, 68)' : 'rgba(239, 68, 68, 0.4)');
    ...
    function renderErrorTypeChart(errorTypes) {
        ...
        const palette = ['#be185d', '#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#6366f1', '#14b8a6', '#f97316', '#e11d48'];
    ...
    function renderAuditorMonthChart(errors, auditorName) {
        ...
        const barColors = ['#be185d', '#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ef4444',
                           '#6366f1', '#14b8a6', '#f97316', '#e11d48', '#84cc16', '#06b6d4', '#9ca3af'];
    ...
    function renderAuditorEvolutionChart(errors, auditorName) {
        ...
        let cumulative = [];
        let acc = 0;
        values.forEach(v => { acc += v; cumulative.push(acc); });
    ...
    function renderErrorTable(reset = false) {
        ...
        if (!currentErrors || currentErrors.length === 0) {
            ...
            tbody.innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-gray-500 italic">No se encontraron inconsistencias. ¡Todo limpio!</td></tr>';
        ...
        const nextBatch = currentErrors.slice(visibleCount, visibleCount + PAGE_SIZE);
        console.log(`[FRONTEND PERF] Rendering batch: current_visible=${visibleCount}, next_count=${nextBatch.length}, total_total=${currentErrors.length}`);
